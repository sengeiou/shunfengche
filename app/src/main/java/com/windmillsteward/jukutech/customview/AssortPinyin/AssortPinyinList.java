package com.windmillsteward.jukutech.customview.AssortPinyin;

import com.windmillsteward.jukutech.bean.CityBean;

import net.sourceforge.pinyin4j.PinyinHelper;

public class AssortPinyinList {
    /**
     * 返回城市首字母列表
     *
     * @return
     */
    public HashList<String, CityBean> getMemberHashList() {
        return MemberHashList;
    }

    /**
     * 城市首字字母hash列表
     */
    private HashList<String, CityBean> MemberHashList = new HashList<String, CityBean>(
            new KeySort<String, CityBean>() {
                public String getKey(CityBean mi) {
                    return getFirstChar(mi.getArea_name());
                }
            });

    /**
     * 获得字符串的首字母 首字符转汉语拼音
     *
     * @param value 需要转换的字符
     * @return
     */
    public String getFirstChar(String value) {
        if (value == null || "".equals(value)) {
            return "?";
        }
        // 首字符
        char firstChar = value.charAt(0);
        // 首字母分类
        String first = null;
        // 是否是非汉字
        String[] print = PinyinHelper.toHanyuPinyinStringArray(firstChar);

        if (print == null) {

            // 将小写字母改成大写
            if ((firstChar >= 97 && firstChar <= 122)) {
                firstChar -= 32;
            }
            if (firstChar >= 65 && firstChar <= 90) {
                first = String.valueOf((char) firstChar);
            } else {
                // 认为首字符为数字或者是特殊字符
                first = "#";
            }
        } else {
            // 如果是中文，分类大写字母
            first = String.valueOf((char) (print[0].charAt(0) - 32));
        }
        if (first == null) {
            first = "?";
            return first;
        }
        return first;
    }
}
