...
public class PositionListPresenter extends BaseNetModelImpl {
    ...
    private final int POSITION_CLASS = 8;
     ...
     * 加载职位分类数据
     */
    public void loadPositionClassData() {
        DataLoader dataLoader = new DataLoader(this, POSITION_CLASS);
        HttpInfo httpInfo = new HttpInfo();
        Map<String, Object> map = new HashMap<>();
        httpInfo.setUrl(APIS.URL_POSITION_CLASS_LIST);
        httpInfo.setParams(map);
        dataLoader.httpPost(httpInfo);
    }
    ...
    protected Type getDataType(int action) {
        ...
        switch (action) {
            ...
            case REFRESH_DATA:
            case INIT_DATA:
                type = new TypeReference<BaseResultInfo<PositionListBean>>() {
                }.getType();
                break;
            case AREA_LIST:
            ...
            case BANNER:
                ...
                break;
            case POSITION_CLASS:
                type = new TypeReference<BaseResultInfo<List<HomePositionClassBean>>>() {
                }.getType();
                break;
        }
        return type;
    ...
    protected void onDataCallback(int code, int action, BaseResultInfo result, String sourceData) {
        switch (action) {
            case INIT_DATA:
                view.dismiss();
                PositionListBean initData = (PositionListBean) result.getData();
                view.initDataSuccess(initData);
                break;
            case REFRESH_DATA:
                PositionListBean refreshData = (PositionListBean) result.getData();
                view.refreshDataSuccess(refreshData);
                break;
            case NEXT_DATA:
                PositionListBean nextData = (PositionListBean) result.getData();
            ...
            case POSITION_CLASS:
                view.dismiss();
                List<HomePositionClassBean> homePositionClassBeanList = (List<HomePositionClassBean>) result.getData();
                if (homePositionClassBeanList!= null){
                view.loadPositionClassSuccess(homePositionClassBeanList);
                }
                break;
