...
import com.windmillsteward.jukutech.activity.chat.db.UserDao;
...
import com.windmillsteward.jukutech.activity.chat.domain.EaseEmojiconGroupEntity;
...
import com.windmillsteward.jukutech.activity.chat.model.EaseNotifier;
import com.windmillsteward.jukutech.activity.chat.parse.UserProfileManager;
import com.windmillsteward.jukutech.activity.chat.receiver.CallReceiver;
import com.windmillsteward.jukutech.activity.chat.receiver.HeadsetReceiver;
...
public class DemoHelper {
    private List<EaseUser> easeUserListFromAppServer;
            ...
            public void onConnected() {
                ...
                if (isGroupsSyncedWithServer && isContactsSyncedWithServer) {
                    ...
                    if (!isContactsSyncedWithServer) {
                        if (!TextUtils.isEmpty(Hawk.get(Define.ACCESS_TOKEN,""))) {
                            getContactListFromAppServer();
                        }
//                        asyncFetchContactsFromServer(null);
    ...
    private void getContactListFromAppServer(){
        new NetUtil().setUrl(APIS.URL_HX_CONTACT_LIST).setCallBackData(new BaseNewNetModelimpl<List<EaseUser>>() {
            ...
            protected void onSuccess(int code, BaseResultInfo<List<EaseUser>> respnse, String source) {
                easeUserListFromAppServer = respnse.getData();
                asyncFetchContactsFromServer(null);
            }

            @Override
            protected java.lang.reflect.Type getType() {
                return new TypeToken<BaseResultInfo<List<EaseUser>>>() {
                }.getType();
            }
        }).buildPost();
    }
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    String userName = message.getStringAttribute(EaseConstant.USER_NICK_NAME, "");
                    String userPic = message.getStringAttribute(EaseConstant.USER_AVATAR_URL, "");
...
//                        notifyForRecevingEvents();
    ...
    public void asyncFetchContactsFromServer(final EMValueCallBack<List<String>> callback) {
        ...
        if (easeUserListFromAppServer == null){
            return;
        }
...
//                    usernames = EMClient.getInstance().contactManager().getAllContactsFromServer();
                    selfIds = EMClient.getInstance().contactManager().getSelfIdsOnOtherPlatform();//从服务器获取登录用户在其他设备上登录的ID
                    // in case that logout already before server returns, we should return immediately
                    if (!isLoggedIn()) {
...
//                    for (EaseUser easeUser:easeUserListFromAppServer){
//                        usernames.add(TextUtils.isEmpty(easeUser.getUsername())?"":easeUser.getUsername());
//                    }
//                    if (selfIds.size() > 0) {
...
//                    }
                    ...
                    for (EaseUser easeUser:easeUserListFromAppServer){
//                        EaseUser user = new EaseUser(easeUser.getUsername());
