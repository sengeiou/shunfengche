...
import com.windmillsteward.jukutech.activity.chat.parse.UserProfileManager;
import com.windmillsteward.jukutech.activity.chat.ui.EaseGroupListener;
...
import com.windmillsteward.jukutech.activity.chat.widget.EaseExpandGridView;
import com.windmillsteward.jukutech.activity.chat.widget.EaseSwitchButton;
...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
    ...
    private List<EaseUser> adminList = Collections.synchronizedList(new ArrayList<EaseUser>());
    private List<EaseUser> memberList = Collections.synchronizedList(new ArrayList<EaseUser>());
    ...
    private List<EaseUser> easeUserList;//服务器返回的成员列表
    private List<String> memberList20;//环信服务器获取到的前20名成员
...
//		st = getResources().getString(R.string.people);
        ...
        membersAdapter = new GridAdapter(this, R.layout.em_grid_owner, new ArrayList<EaseUser>());
        EaseExpandGridView userGridview = (EaseExpandGridView) findViewById(R.id.gridview);
        userGridview.setAdapter(membersAdapter);

        ownerAdminAdapter = new OwnerAdminAdapter(this, R.layout.em_grid_owner, new ArrayList<EaseUser>());
    ...
    private void getOwnerListFromAppServer() {
        showDialog();
        final List<String> admins = group.getAdminList();//todo 此处获取群管理列表,不包括群主
        admins.add(0, group.getOwner());
        StringBuffer stringBuffer = new StringBuffer();
        for (int i = 0; i < admins.size(); i++) {
            if (i == 0) {
                stringBuffer.append(admins.get(i));
            } else {
                stringBuffer.append("," + admins.get(i));
            }
        }
        addCall(new NetUtil().setUrl(APIS.URL_HX_USER_INFO_LIST)
                .addParams("usernames", stringBuffer.toString())
                .setCallBackData(new BaseNewNetModelimpl<List<EaseUser>>() {
                    @Override
                    ...
                    @Override
                    protected void onSuccess(int code, BaseResultInfo<List<EaseUser>> respnse, String source) {
                        dismiss();
                        adminList.clear();
                        List<EaseUser> easeUserList = respnse.getData();
                        if (easeUserList != null) {
                            for (int i = 0; i < easeUserList.size(); i++) {//和服务器的进行比较
                                EaseUser easeUser = easeUserList.get(i);
                                for (int j = 0; j < admins.size(); j++) {//如果用户名相同，则add
                                    if (easeUser.getUsername().equals(admins.get(j))) {
                                        adminList.add(easeUser);
                                    }
                                }
                            }
                        ...
                        getMembersListFromAppServer();

                    }

                    @Override
                    protected Type getType() {
                        return new TypeReference<BaseResultInfo<List<EaseUser>>>() {
                        }.getType();
                    }
                }).buildPost());
    }
     ...
     * 从app服务器获取群成员列表
     */
    private void getMembersListFromAppServer() {
        StringBuffer sb = new StringBuffer();
        if (memberList20 != null) {
            for (int i = 0; i < memberList20.size(); i++) {
                if (i == 0) {
                    sb.append(memberList20.get(i));
                } else {
                    sb.append("," + memberList20.get(i));
                }
            }
        }
//        final List<String> members = group.getMembers();//todo 此处获取不到群成员列表
//        StringBuffer stringBuffer = new StringBuffer();
...
//        }
        ...
        addCall(new NetUtil().setUrl(APIS.URL_HX_USER_INFO_LIST)
                .addParams("usernames", sb.toString())
                .setCallBackData(new BaseNewNetModelimpl<List<EaseUser>>() {
                    @Override
                    protected void onFail(int type, String msg) {
                        showTips(msg);
                    ...
                    @Override
                    protected void onSuccess(int code, final BaseResultInfo<List<EaseUser>> respnse, String source) {
                        dismiss();
                        memberList.clear();
                        List<EaseUser> easeUserList = respnse.getData();
                        if (easeUserList != null) {
                            for (int i = 0; i < easeUserList.size(); i++) {
                                EaseUser easeUser = easeUserList.get(i);
                                for (int k = 0; k < memberList20.size(); k++) {
                                    if (easeUser.getUsername().equals(memberList20.get(k))) {
                                        memberList.add(easeUser);
                                    }
                                }
                            }
                        }
    ...
    boolean isCurrentAdmin(EMGroup group) {
        synchronized (adminList) {
            ...
            for (EaseUser easeUser : adminList) {
                if (currentUser.equals(easeUser.getUsername())) {
                    return true;
                }
            }
        }
    ...
    boolean isAdmin(String id) {
        synchronized (adminList) {
            for (EaseUser easeUser : adminList) {
                if (id.equals(easeUser.getUsername())) {
                    return true;
                }
            }
        }
...
//                debugList("blackList", blackList);
                ...
                membersAdapter = new GridAdapter(GroupDetailsActivity.this, R.layout.em_grid_owner, new ArrayList<EaseUser>());
...
//                }
    ...
    private void deleteGrop() {
        ...
        addCall(new NetUtil().setUrl(APIS.URL_Exit_GROUP)
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.rl_change_group_name:
                startActivityForResult(new Intent(this, ChatEditActivity.class).putExtra("data", group.getGroupName()).putExtra("editable", isCurrentOwner(group)),
                        REQUEST_CODE_EDIT_GROUPNAME);
                break;
            case R.id.rl_change_group_description:
                startActivityForResult(new Intent(this, ChatEditActivity.class).putExtra("data", group.getDescription()).
...
//				startActivity(new Intent(this, SharedFilesActivity.class).putExtra("groupId", groupId));
            ...
            case R.id.rl_change_group_extension:
                startActivityForResult(new Intent(this, ChatEditActivity.class).putExtra("data", group.getExtension()).
...
//		final ArrayList list = (ArrayList) Arrays.asList(groupId);
     ...
     * @author admin_new
     */
    private class OwnerAdminAdapter extends ArrayAdapter<EaseUser> {

        private int res;

        public OwnerAdminAdapter(Context context, int textViewResourceId, List<EaseUser> objects) {
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            ...
            final EaseUser easeUser = getItem(position);
            convertView.setVisibility(View.VISIBLE);
            button.setVisibility(View.VISIBLE);
            holder.textView.setText(easeUser.getNickname());
            GlideUtil.show(getContext(), easeUser.getAvatar(), holder.imageView);
//            EaseUserUtils.setUserNick(easeUser.getUsername(), holder.textView);
//            EaseUserUtils.setUserAvatar(getContext(), easeUser.getUsername(), holder.imageView);
            ...
            id_background.setBackgroundColor(convertView.getResources().getColor(
                    position == 0 ? R.color.holo_red_light :
                            (isInMuteList(easeUser.getUsername()) ? R.color.gray_normal : R.color.holo_orange_light)));
            ...
            button.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (easeUser.getUsername().equals(group.getOwner())) {
                        return;
                    }
                    operationUserId = easeUser.getUsername();
...
//				}
                ...
                button.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        boolean inBlackList = isInBlackList(easeUser.getUsername());
                        boolean inMuteList = isInMuteList(easeUser.getUsername());
                        try {
                            if (inBlackList) {
            ...
            public void run() {
                try {
                    ...
                    try {
                        ...
                        memberList20 = result.getData();
                        getOwnerListFromAppServer();//先获取管理员再获取群成员
...
//                        memberList.removeAll(adminList);
                    ...
                    try {
                        EMClient.getInstance().groupManager().fetchGroupAnnouncement(groupId);//获取群公告
                    } catch (HyphenateException e) {
                        e.printStackTrace();
                    }
