...
public class WalletDetailListPresenter extends BaseNetModelImpl {
    ...
    private String[] wallet_cost_type_module = new String[]{"全部明细","收入","支出","退款","提现"};

    private WalletListView view;

    public WalletDetailListPresenter(WalletListView view) {
        this.view = view;
    }
    ...
    public void loadWalletCostTypeData(){
        ...
        for (int i = 0; i < wallet_cost_type_module.length; i++) {
            Map<String,Object> map = new HashMap<>();
            map.put("id",i);
            map.put("text",wallet_cost_type_module[i]);
            maps.add(map);
        }
        view.loadWalletCostTypeDataSuccess(maps);
    }
