 ...
 * 作者：xjh
 */
public class ChoiceIdeaThinkClassPresenter extends BaseNetModelImpl {

    private static final int INIT_DATA = 1;

    private ChoiceIdeaThinkClassView view;

    public ChoiceIdeaThinkClassPresenter(ChoiceIdeaThinkClassView view) {
        this.view = view;
    }
    ...
    protected void onDataCallback(int code, int action, BaseResultInfo result, String sourceData) {
        switch (action) {
            case INIT_DATA:
                view.dismiss();
                List<IdeaThinkClassBean> data = (List<IdeaThinkClassBean>) result.getData();
                if(data!=null) {
